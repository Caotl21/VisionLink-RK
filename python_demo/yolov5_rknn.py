#!/home/AUV1/miniconda3/bin/python
import cv2
import numpy as np
import time
...
class RKNN_Inference:
    ...
    def run(self, img):
        ...
        self.img_rgb = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB)
        
        # 增加 batch 维度 (如果模型需要) 或者直接作为列表传入
        # rknnlite 通常接受 [H, W, C] 格式的输入列表
        inputs = [self.img_rgb]
...
def main():
    # 初始化 RKNN
    rknn_lite = RKNN_Inference(RKNN_MODEL)
    ...
    try:
        while True:
            ...
            # 后处理 (需要根据具体模型实现)
            #detections = post_process(outputs, frame.shape)
            boxes, classes, scores = rknn_lite.yolov5_post_process(input_data)
            ...
            if frame_count >= 30:
                ...
                fps_time = time.time()
                frame_count = 0
                print(f"FPS: {fps:.2f}, Inference: {inference_time:.2f}ms")

            img_1 = cv2.cvtColor(rknn_lite.img_rgb, cv2.COLOR_RGB2BGR)
            if boxes is not None:
                rknn_lite.draw(img_1, boxes, scores, classes)
            # show output
